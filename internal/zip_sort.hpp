...
#ifndef INTERNAL_ZIP_SORT_H_
#define INTERNAL_ZIP_SORT_H_
...
FAST_TOPK_INLINE size_t MedianOf3(Comparator comp, T iter, size_t index1, size_t index2) {
  ...
  prefetch<PREFETCH_HINT_NTA>(&mid);
  const auto &first = iter[index1];
  prefetch<PREFETCH_HINT_NTA>(&first);
  const auto &last = iter[index2 - 1];
  prefetch<PREFETCH_HINT_NTA>(&last);
...
void PrefetchIter(T iter) {
  prefetch<PREFETCH_HINT_NTA>(&(*iter));
}
...
FAST_TOPK_INLINE size_t PivotPartition(Comparator comp, size_t index1, size_t index2,
  ...
  while (true) {
    ...
    if (FAST_TOPK_PREDICT_TRUE(less_iter < greater_iter)) {
...
size_t PivotPartitionBranchOptimized(Comparator comp, size_t index1,
  ...
  for (;;) {
    ...
    do {
      ...
      if (FAST_TOPK_PREDICT_FALSE(left_swaps_slot == 0)) {
        if (FAST_TOPK_PREDICT_FALSE(right_swaps_slot == 0)) break;
        while (FAST_TOPK_PREDICT_TRUE(index1 < index2)) {
          right_swaps.end()[right_swaps_slot] = index2;
          if ((right_swaps_slot += !comp(pivot, begin[index2--])) == 0) break;
        }
        break;
      } else if (FAST_TOPK_PREDICT_FALSE(right_swaps_slot == 0)) {
        while (FAST_TOPK_PREDICT_TRUE(index1 < index2)) {
          left_swaps.end()[left_swaps_slot] = index1;
    ...
    if (FAST_TOPK_PREDICT_FALSE(index1 >= index2)) break;
...
template <typename Comparator, typename T, typename... U>
FAST_TOPK_ATTRIBUTE_NOINLINE void ZipMakeHeap(Comparator comp, size_t index1,
...
template <typename Comparator, typename T, typename... U>
FAST_TOPK_ATTRIBUTE_NOINLINE void ZipHeapSort(Comparator comp, size_t index1,
                                              size_t index2, T begin,
                                              U... rest) {
...
FAST_TOPK_INLINE size_t ComputeDepthLimit(const size_t range_size) {
  ...
  if (range_size == 0) {
    ...
    return 2 * Log2Ceiling(range_size);
  }
}

template <typename Comparator, typename T, typename... U>
FAST_TOPK_ATTRIBUTE_NOINLINE void ZipSort(Comparator comp, size_t index1,
  ...
  while (true) {
    if (index2 - index1 < kInsertionSortSize) {
      ...
      ZipHeapSort(comp, index1, index2, begin, rest...);
      return;
    }
    ...
    if (index2_greater - index1_greater > index2_less - index1_less) {
      ZipSort(comp, index1_less, index2_less, depth_limit, begin, rest...);
      index1 = index1_greater;
      index2 = index2_greater;
    } else {
      ZipSort(comp, index1_greater, index2_greater, depth_limit, begin,
              rest...);
      index1 = index1_less;
      index2 = index2_less;
    }
...
template <typename Comparator, typename T, typename... U>
FAST_TOPK_ATTRIBUTE_NOINLINE void ZipSortBranchOptimized(Comparator comp,
                                                         size_t index1,
                                                         size_t index2,
                                                         size_t depth_limit,
                                                         T begin, U... rest) {
  while (true) {
    ...
    if (index2_greater - index1_greater > index2_less - index1_less) {
      ZipSortBranchOptimized(comp, index1_less, index2_less, depth_limit, begin,
                             rest...);
      index1 = index1_greater;
      index2 = index2_greater;
    } else {
      ZipSortBranchOptimized(comp, index1_greater, index2_greater, depth_limit,
                             begin, rest...);
      index1 = index1_less;
      index2 = index2_less;
    }
