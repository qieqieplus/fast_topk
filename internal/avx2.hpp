...
#ifndef INTERNAL_AVX2_H_
#define INTERNAL_AVX2_H_
...
constexpr size_t InferNumRegisters() {
  ...
  static_assert(kNumElementsRequired % kElementsPerRegister == 0);
...
class Avx2<T, kNumRegistersInferred> {
 public:
  ...
  FAST_TOPK_AVX2_INLINE Avx2(Avx2Zeros) { Clear(); }

  FAST_TOPK_AVX2_INLINE Avx2(IntelType val) {
    static_assert(kNumRegisters == 1);
    *this = val;
  }

  FAST_TOPK_AVX2_INLINE Avx2(T val) { *this = Broadcast(val); }

  template <typename U, size_t kOther>
  FAST_TOPK_AVX2_INLINE explicit Avx2(const Avx2<U, kOther> &other) {
    Avx2 &me = *this;
  ...
  /*
  FAST_TOPK_AVX2_INLINE Avx2(Avx1<T, kNumRegisters> avx1) {
    Avx2& me = *this;
    for (size_t j = 0; j <  kNumRegisters; j++) {
      me[j] = *avx1[j];
    }
  ...
  FAST_TOPK_AVX2_INLINE operator Avx1<T, kNumRegisters>() const {
    const Avx2& me = *this;
  ...
  FAST_TOPK_AVX2_INLINE Avx2 &operator=(Avx2Zeros val) {
    Clear();
    return *this;
  }

  FAST_TOPK_AVX2_INLINE Avx2 &operator=(IntelType val) {
    static_assert(kNumRegisters == 1,
  ...
  FAST_TOPK_AVX2_INLINE Avx2 &operator=(T val) {
    *this = Broadcast(val);
    return *this;
  }

  FAST_TOPK_AVX2_INLINE IntelType operator*() const {
    static_assert(kNumRegisters == 1);
    return registers_[0];
  }

  FAST_TOPK_AVX2_INLINE Avx2<T, 1> &operator[](size_t idx) {
  ...
  FAST_TOPK_AVX2_INLINE const Avx2<T, 1> &operator[](size_t idx) const {
    if constexpr (kNumRegisters == 1) {
      // DCHECK_EQ(idx, 0);
      return *this;
    } else {
  ...
  static FAST_TOPK_AVX2_INLINE IntelType ZeroOneRegister() {
  ...
  static FAST_TOPK_AVX2_INLINE Avx2 Zeros() {
  ...
  FAST_TOPK_AVX2_INLINE Avx2 &Clear() {
    for (size_t j = 0; j < kNumRegisters; j++) {
  ...
  static FAST_TOPK_AVX2_INLINE IntelType BroadcastOneRegister(T x) {
    if constexpr (IsSameAny<T, float>()) {
  ...
  FAST_TOPK_AVX2_INLINE static Avx2 Broadcast(T x) {
    Avx2 ret;
    for (size_t j = 0; j < kNumRegisters; j++) {
  ...
  static FAST_TOPK_AVX2_INLINE IntelType LoadOneRegister(const T *address) {
  ...
  template <bool kAligned = false>
  FAST_TOPK_AVX2_INLINE static Avx2 Load(const T *address) {
    Avx2 ret;
    for (size_t j = 0; j < kNumRegisters; j++) {
  ...
  static FAST_TOPK_AVX2_INLINE void StoreOneRegister(T *address, IntelType x) {
    if constexpr (IsSameAny<T, float>()) {
  ...
  FAST_TOPK_AVX2_INLINE void Store(T *address) const {
    const Avx2 &me = *this;
    for (size_t j = 0; j < kNumRegisters; j++) {
  ...
  FAST_TOPK_AVX2_INLINE array<T, kNumElements> Store() const {
  ...
  static FAST_TOPK_AVX2_INLINE Avx2<T, kOutput> BinaryOperatorImpl(
      const Avx2 &me, const Avx2<T, kOther> &other, Op fn) {
    Avx2<T, kOutput> ret;
    for (size_t j = 0; j < Avx2<T, kOutput>::kNumRegisters; j++) {
      if constexpr (kOther == kNumRegisters) {
  ...
  static FAST_TOPK_AVX2_INLINE IntelType Add(IntelType a, IntelType b) {
    if constexpr (IsSameAny<T, float>()) {
  ...
  FAST_TOPK_AVX2_INLINE auto operator+(const Avx2<T, kOther> &other) const {
    return BinaryOperatorImpl(*this, other, &Add);
  }

  static FAST_TOPK_AVX2_INLINE IntelType Subtract(IntelType a, IntelType b) {
    if constexpr (IsSameAny<T, float>()) {
  ...
  static FAST_TOPK_AVX2_INLINE IntelType Multiply(IntelType a, IntelType b) {
  ...
  static FAST_TOPK_AVX2_INLINE IntelType Divide(IntelType a, IntelType b) {
    if constexpr (IsSame<T, float>()) {
  ...
  static FAST_TOPK_AVX2_INLINE auto BitwiseAnd(IntelType a, IntelType b) {
    if constexpr (IsSame<T, float>()) {
  ...
  static FAST_TOPK_AVX2_INLINE auto BitwiseOr(IntelType a, IntelType b) {
    if constexpr (IsSame<T, float>()) {
  ...
  static FAST_TOPK_AVX2_INLINE IntelType ShiftRight(IntelType x, int count) {
  ...
  FAST_TOPK_AVX2_INLINE Avx2 operator>>(int count) const {
    const Avx2 &me = *this;
    Avx2 ret;
    for (size_t j = 0; j < kNumRegisters; j++) {
  ...
  static FAST_TOPK_AVX2_INLINE IntelType ShiftLeft(IntelType x, int count) {
    static_assert(!IsSameAny<T, int8_t, uint8_t>(),
                  "There's no 8-bit '<<' instruction");
    static_assert(!IsSameAny<T, float, double>(),
                  "Bit shifting isn't defined for floating-point types.");
  ...
  template <size_t kOther, typename Op>
  FAST_TOPK_AVX2_INLINE Avx2 &AccumulateOperatorImpl(
      const Avx2<T, kOther> &other, Op fn) {
    Avx2 &me = *this;
    for (size_t j = 0; j < kNumRegisters; j++) {
      if constexpr (kOther == kNumRegisters) {
  ...
  template <size_t kOther>
  FAST_TOPK_AVX2_INLINE Avx2 &operator+=(const Avx2<T, kOther> &other) {
  ...
  FAST_TOPK_AVX2_INLINE Avx2 &operator<<=(int count) {
    Avx2 &me = *this;
    for (size_t j = 0; j < kNumRegisters; j++) {
  ...
  FAST_TOPK_AVX2_INLINE auto ComparisonOperatorImpl(
      const Avx2 &me, const Avx2<T, kOther> &other, Op fn) const {
    Avx2<T, std::max(kNumRegisters, kOther)> masks;
    for (size_t j = 0; j < std::max(kNumRegisters, kOther); j++) {
      if constexpr (kOther == kNumRegisters) {
  ...
  static FAST_TOPK_AVX2_INLINE IntelType LessThan(IntelType a, IntelType b) {
    if constexpr (IsSameAny<T, float>()) {
  ...
  static FAST_TOPK_AVX2_INLINE IntelType LessOrEquals(IntelType a,
                                                      IntelType b) {
    if constexpr (IsSameAny<T, float>()) {
  ...
  static FAST_TOPK_AVX2_INLINE IntelType Equals(IntelType a, IntelType b) {
    if constexpr (IsSameAny<T, float>()) {
  ...
  static FAST_TOPK_AVX2_INLINE IntelType GreaterOrEquals(IntelType a,
                                                         IntelType b) {
    if constexpr (IsSameAny<T, float>()) {
  ...
  static FAST_TOPK_AVX2_INLINE IntelType GreaterThan(IntelType a, IntelType b) {
    if constexpr (IsSameAny<T, float>()) {
  ...
  FAST_TOPK_AVX2_INLINE int MaskFromHighBits() const {
    static_assert(kNumRegisters == 1);
    const auto &me = *this;
  ...
  FAST_TOPK_AVX2_INLINE T GetLowElement() const {
    static_assert(kNumRegisters == 1);
    const auto &me = *this;
  ...
  template <typename U>
  static FAST_TOPK_AVX2_INLINE typename Avx2<U>::IntelType ConvertOneRegister(
      IntelType x) {
  ...
  template <typename U>
  FAST_TOPK_AVX2_INLINE Avx2<U, kNumRegisters> ConvertTo() const {
    const Avx2 &me = *this;
  ...
  static FAST_TOPK_AVX2_INLINE auto InferExpansionType() {
    if constexpr (IsSame<T, float>()) {
      return double();
    }
    if constexpr (IsSame<T, double>()) {
  ...
  static FAST_TOPK_AVX2_INLINE pair<ExpansionIntelType, ExpansionIntelType>
  ...
  FAST_TOPK_AVX2_INLINE ExpandsTo ExpandTo() const {
...
class Avx2 {
 public:
  ...
  FAST_TOPK_AVX2_INLINE Avx2(Avx2Zeros) {
    for (size_t j = 0; j < kTensorNumRegisters0; j++) {
      tensor_[j] = Avx2Zeros();
    }
  }

  FAST_TOPK_AVX2_INLINE SimdSubArray &operator[](size_t idx) {
    // DCHECK_LT(idx, kTensorNumRegisters0);
    return tensor_[idx];
  }

  FAST_TOPK_AVX2_INLINE const SimdSubArray &operator[](size_t idx) const {
    // DCHECK_LT(idx, kTensorNumRegisters0);
    return tensor_[idx];
  }
  ...
  FAST_TOPK_AVX2_INLINE StoreResultType Store() const {
    StoreResultType ret;
    for (size_t j = 0; j < kTensorNumRegisters0; j++) {
...
FAST_TOPK_AVX2_INLINE Avx2<T, index_sequence_sum_v<kNumRegisters...>>
Avx2Concat(const Avx2<T, kNumRegisters> &...inputs) {
  Avx2<T, index_sequence_sum_v<kNumRegisters...>> ret;

  size_t idx = 0;
  auto assign_one_input = [&](auto input) FAST_TOPK_AVX2_INLINE_LAMBDA {
    for (size_t jj = 0; jj < decltype(input)::kNumRegisters; jj++) {
      ret[idx++] = input[jj];
    }
  };
...
FAST_TOPK_AVX2_INLINE uint32_t GetComparisonMask(Avx2<int16_t> a,
                                                 Avx2<int16_t> b) {
...
FAST_TOPK_AVX2_INLINE uint32_t GetComparisonMask(Avx2For<int16_t, 32> cmp) {
  static_assert(Avx2For<int16_t, 32>::kNumRegisters == 2);
  return GetComparisonMask(cmp[0], cmp[1]);
}

FAST_TOPK_AVX2_INLINE uint32_t GetComparisonMask(Avx2<int16_t> cmp[2]) {
  return GetComparisonMask(cmp[0], cmp[1]);
}

FAST_TOPK_AVX2_INLINE uint32_t GetComparisonMask(Avx2<float> cmp) {
  return _mm256_movemask_ps(*cmp);
}

FAST_TOPK_AVX2_INLINE uint32_t GetComparisonMask(Avx2<float> v0, Avx2<float> v1,
                                                 Avx2<float> v2,
                                                 Avx2<float> v3) {
...
FAST_TOPK_AVX2_INLINE uint32_t GetComparisonMask(Avx2<float, 4> cmp) {
  return GetComparisonMask(cmp[0], cmp[1], cmp[2], cmp[3]);
}

namespace avx2 {

// FAST_TOPK_INLINE bool RuntimeSupportsSimd() { return RuntimeSupportsAvx2(); }
